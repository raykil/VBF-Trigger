...
if whichfiles=="singlemuon":
    ...
    path = "/Users/raymondkil/Desktop/vbftrigger/run3parqWjetinfo/"
    ...
    OFFJets = ak.from_parquet(path + "new_OFFJets.parquet")
    HLTJets = ak.from_parquet(path + "new_HLTJets.parquet")
    MuonCollections = ak.from_parquet(path + "new_MuonCollections.parquet")
    TrigObjs = ak.from_parquet(path + "new_TrigObjs.parquet")
    OFFJets = vector.zip({
            "pt": OFFJets.rho,
            "eta": OFFJets.eta,
            "phi": OFFJets.phi,
...
HLTCandJets, HLTCandMaxMjjComo = vbf.SelectHLTJetsCand(cleanOFFJets, cleanOFFcombo)
...
### POSTPROCESSING ###
vardict = vbf.GetVardict(HLTPassed_OFFJets, HLTCandJets, analysis, triggerdict)
