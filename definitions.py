...
def AssignFilterBitsToOFFJets(TrigObjs, OFFJets, HLTJets, triggerpath):
    ...
    print(f"Number of events with >2 jets: {len(HLTPassed_OFFJets)}")
...
def GetVardict(HLTPassed_OFFJets, HLTCandJets, analysis, triggerdict):
    if analysis=="LeadJetPtAnalysis":
        vardict = {
            "plotname": "leadpt",
            "binsize"  : 5,
    ...
    if analysis=="MjjAnalysis":
        passed_mjj = GetMaxMjj(HLTPassed_OFFJets)[-1]
        # If this is zero, then number of
        print("nEvents with more than one mjjs: ", ak.count_nonzero(ak.where(ak.num(passed_mjj)>1, 1, 0)))
        cand_mjj = GetMaxMjj(HLTCandJets)[-1]
        vardict = {
            "plotname": "mjj",
            "binsize"  : 50,
